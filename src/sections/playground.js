...
export function renderIDE(lang, translations) {
  ...
  section.innerHTML = `
    <h2 class="rainbow-title-center reveal" style="color: var(--text-primary); font-family: var(--font-mono);">Interactive Playground</h2>
    
    <div class="ide-window reveal" id="ide-window">
      <div class="ide-header">
        <div class="window-controls">
          <div class="control close" id="win-close" title="Close Playground"></div>
          <div class="control minimize" id="win-min" title="Collapse"></div>
          <div class="control maximize" id="win-max" title="Full Screen"></div>
        </div>
  ...
  setTimeout(async () => {
    ...
    // Create and Inject Launch & Restore buttons (Restoration from Evolved State)
    ...
    // Auto-scroll logic for immersive experience
    const scrollToIDE = (delay = 100) => {
        setTimeout(() => {
            // Using offsetTop for more reliable positioning during viewport transitions
            const yOffset = -20; 
    ...
    section.querySelector('#win-min').onclick = () => {
        ...
        setTimeout(() => {
            ...
            restoreFab.style.animation = 'popIn 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)';
        }, 400);
    };
    ...
    // Listen for fullscreen exit to scroll back - ONLY if it was the IDE that was fullscreen
    document.addEventListener('fullscreenchange', () => {
        // We only want to scroll to IDE if the IDE window itself was the fullscreen element
        // or if we are exiting a state where the IDE should be in focus.
        // To avoid conflicts with project modals, we'll check if the project modal exists.
        const isProjectModalActive = !!document.getElementById('project-preview-modal');
        
        if (!document.fullscreenElement && !isProjectModalActive) {
            // Only scroll if we aren't closing a project preview
            scrollToIDE(500); 
        }
    });
